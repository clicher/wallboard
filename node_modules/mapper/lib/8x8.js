...
function makeUrl(bf, callback) {
    ...
    if(diff == 0 || bf) {
        ...
        p.mapData.dateRange = start.format('YYYY-MM-DD') + ',' + moment().format('YYYY-MM-DD');
...
function httpRequest(callback) {
    var activity = [];
    console.log(p.protocol + p.host + p.mapData.uri + p.mapData.dateRange + '&n=' + String(p.mapData.paginationStart));
    request(
        {
            method: p.method,
            uri: p.protocol + p.host + p.mapData.uri + p.mapData.dateRange + '&n=' + String(p.mapData.paginationStart),
