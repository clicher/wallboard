  ...
  Location.find({}).
      ...
      exec(function(err, lastLocation) {
        if (err) {
          ...
          lastDate = new moment(lastLocation[0]['trans-accept-time']).format(
              'YYYY-MM-DD');
        } else {
          lastDate = new moment();
        }
...
function gatherDataLoop(callback) {
  x8(lastDate, config.phoneSystemInformationForMap, backfill, function(err, d) {
    ...
    findIfExists(d, function(err, da) {
      ...
      areaCodeLookup(da, config.areaCodeApiInformationForMap,
          function(err, dat) {
...
function bulkUpdateDb(dat, callback) {
  ...
  async.eachSeries(dat, function(l, cb) {
    var query = {
      'trans-accept-time': l['trans-accept-time'],
      'origination': l['origination'],
    };
    var location = new Location(l);
    Location.findOneAndUpdate(query, {$set: location}, {upsert: true},
        function(err, loc) {
          console.log(loc);
          cb();
        });
